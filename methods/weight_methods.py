...
class LinearScalarization(WeightMethod):
    ...
    def update_weights(self, task_weights: Union[List[float], torch.Tensor]):
        if not isinstance(task_weights, torch.Tensor):
            task_weights = torch.tensor(task_weights)
        assert len(task_weights) == self.n_tasks
        self.task_weights = task_weights.to(self.device)
...
class GradCosine(WeightMethod):
    """Implementation of the unweighted version of the alg. in 'Adapting Auxiliary Losses Using Gradient Similarity'"""

    def __init__(self, n_tasks: int, device: torch.device, main_task):
        super(GradCosine, self).__init__(n_tasks=n_tasks, device=device)
        self.main_task = main_task
...
class AuxiLearn(WeightMethod):
    """Auxiliary learning by implicit differentiation"""

    def __init__(self, n_tasks: int, device: torch.device, main_task, linear=False):
        super().__init__(n_tasks=n_tasks, device=device)
        self.main_task = main_task
        self.linear = linear
        if linear:
            self.net = MonoLinearHyperNet(
                main_task=main_task, input_dim=n_tasks, weight_normalization=False
            ).to(device)
        else:
            self.net = MonoNonlinearHyperNet(
                main_task=main_task,
                hidden_sizes=[3, 3],
                input_dim=n_tasks,
    ...
    def get_weighted_loss(
        ...
        **kwargs,
        ...
        self.clamp()  # make sure weights are non-negative
        extra_outputs = dict()
        if self.linear:
            extra_outputs = dict(
                weights=self.net.linear.weight.clone().detach().flatten()
            )
        return self.net(losses), extra_outputs
    ...
    def backward(
        ...
        **kwargs,
        ...
        loss, extra_outputs = self.get_weighted_loss(losses, shared_parameters)
        loss.backward()
        return loss, extra_outputs

    def parameters(self) -> Union[Iterable[nn.Parameter], List[nn.Parameter]]:

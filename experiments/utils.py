...
common_parser.add_argument(
    "--method-params-lr",
    type=float,
    default=5e-3,  # NOTE: similar to meta-lr
...
common_parser.add_argument(
    ...
    type=lambda s: None
    if s is None
    else [float(item.strip()) for item in s.split(",")],
    default=None,
)
# stl
common_parser.add_argument(
...
common_parser.add_argument("--wandb-project", type=str, help="wandb project name")
...
def extract_weight_method_parameters_from_args(args):
    ...
    weight_methods_parameters.update(
        dict(
            ...
            gcs=dict(main_task=args.main_task),
            olaux=dict(
                main_task=args.main_task, update_weights_every=args.update_weights_every
            ),
            ...
            auxilearn=dict(main_task=args.main_task, linear=args.auxilearn_linear),
        )
    )
